function create_netlist(name, path)
    nl = open(path * '/' * name * "_nl.csv", "w")               #Create file containing TOPOLOGY PARAMETERS OF SQUID APF CIRCUIT
    cp = open(path * '/' * name * "_cp.txt", "w")  #Create file containing COMPONENT PARAMETERS OF SQUID APF CIRCUIT PARAMETER
    lc = open(path * '/' * name * "_cp.csv", "w")      #Create file containing LOOPS AND COMPONENTS CONTAINED IN EACH LOOP
